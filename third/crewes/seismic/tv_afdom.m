...
% TV_AFDOM: This is a vectorized version of tvfdom that computes fdom, BW, and Amp
%
...
%
% NOTE: tvfdom produces identical results to tv_afdom or tvfdom3D but the latter are vectorized for
...
ind=find(afd<0);%this happens because of the interpolation
if(~isempty(ind))
    fd(ind)=0;
    afd(ind)=0;
    bwfd(ind)=0;
end
ind=find(bwfd<0);%this happens because of the interpolation
if(~isempty(ind))
    fd(ind)=0;
    afd(ind)=0;
    bwfd(ind)=0;
