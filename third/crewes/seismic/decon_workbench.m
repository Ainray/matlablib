...
% Actions
%   'init' >> initialize
%   'algorithm' >> set the current decon algorithm and show parameters panel
%   'apply' >> apply the current algorithm to the data panel, save in results, call 'welltie'
%   'selectresult' >> select which result to show. Fills panel, plots result from results struc.
%   'deleteresult' >> delete the current result
%   'dragline' >> called to chage the desin design window
%   'recompspectra' >> recompute the spectra, calls 'selectresult'
%   'designgatechangeline' >> change the designgate by line dragging
%   'designgatechangetext' >> change the designgate by text entry
%   'correlationgatechange' >> change the correlation gate, call 'welltie'
%   'clipout' >> change clip on output, calls 'selectresult'
%   'welltie' >> calls welltie.m to tie the well. calls estimate_wavelets (internal). 
%   'waveletmethod' >> changes the wavelet estimation method
%   'recomputewavelets' >> recompute the wavelets
%   'applyfilter' >> apply the current filter specs, recomputespectra, redo welltie
%   'spectrafmax' >> set fmax on spectra
%   'waveletlegend' >> toggle the wavelet legend
%   'comparewavelets' >> compares wavelets
%   'comparewaveletschange' >> ???
%   'waveexmatch' >> switch to match filter, call waveex_match
%   'waveexrw' >> switch to roy white, call waveex_rw
%   'waveexsimple' >> switch to simple, call waveex_simple
%   'waveex' >> calls current waveex method
%   'alignment' >> calls align_ref
%   'newref' >> receives new reflectivity from align_ref
%   'switchref' >> chance the reflectivity
%
% Internal functions
%   populatepanel(hpanel,alg,algorithm) creates and fills the decon parameters panel
%   fillpanel(thisalg,hpanel,parms) fills and already existing decon panel
%   fillfilterpanel(parms) fills in the filter panel
%   fillwaveletpanel(method,parms) fills in the wavelet estimation panel
%   thisresult=getcurrentresult; Get the current result
%   savecurrentresult(thisresult); save the current result
%   plotresult(seisd,t,x,sref,tref,xref,kol) plot a result
%   plot_tf(seis,t,x,sref,tref,xref,kol,hseis,hfreq,clip,flag_t,flag_f) some kinda plotting
%   plot_f(seis,t,hfreq,flag_f) some other kinda plotting
%   plotwelltie(t,tvphase,tvdelay,sreftie,tref,s1tie,twin,kol) plot the well tie
...
%   setdesigngate(from,td1,td2) set the design gate
...
%   estimate_wavelets estimate wavelets with the current method (simple, match, rw)
%   [wlets,tws,t0s,twin]=compare_wavelets(s,t,sref,tref) compare wavelets
