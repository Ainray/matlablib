...
%     brighten(.5);
    ...
    uicontrol(hfig,'style','pushbutton','string','Reset windows to globals','units','normalized',...
        'position',[xnow,ynow,1.7*wid,.5*ht],'callback','seisplotfx(''resetwindows'')','tag','resetwin',...
...
%results popup
    ...
    if(flag==0)
        ...
        set(hfig,'name',['F-X amplitude analysis for ' dname2],'closerequestfcn','seisplotfx(''close'');');
    else
        if(~iscell(dname))
            dname2=dname;
        else
        ...
        set(hfig,'name',['F-X phase analysis for ' dname2],'closerequestfcn','seisplotfx(''close'');');
    end
