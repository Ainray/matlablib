...
elseif(strcmp(action,'datainfo'))
    ...
    hpan1=uipanel('title','Text Header','position',[.1 .1 .8 .67]);
    ...
    hpan2=uipanel('title','Data summary','position',[.1 .85 .8 .1]);
...
elseif(strcmp(action,'showtraceheaders'))
    %called by Multiple SEGY load dialog or from Datainfo window
...
hpan=uipanel(hdial,'position',[xnow,.2,.8,.67]);
xn=.02;yn=.9;h=1.5*ht;
...
npanels=length(hpanels)+1;
panelwidth=geom(1);panelheight=1.1*geom(2);xnow=geom(3);ynow=geom(4);
wid=geom(5);ht=geom(6);xsep=geom(7);ysep=geom(8);
ynow=ynow-panelheight-ysep;
hpan=uipanel(hdial,'tag',['data_panel' int2str(npanels)],'units','normalized',...
    'position',[xnow ynow panelwidth panelheight],'userdata',npanels);
geom(4)=ynow;
hpanels{npanels}=hpan;
set(hmpan,'userdata',{hpanels geom []});
...
uicontrol(hpan,'style','pushbutton','string','Trace headers','units','normalized','position',...
    [xn,yn,1.25*wid2,ht],'callback','sane(''showtraceheaders'');','tag','trchdr',...
