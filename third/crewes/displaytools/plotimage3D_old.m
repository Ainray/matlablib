...
function spdcmp(~,~) %spectral decomp
hthisfig=gcf;
hprevious=findobj(gcf,'tag','previous');
udat=get(hprevious,'userdata');
data=udat{1};
...
while ~ok
    ...
    a={'0.02',num2str(2*dt),time2str(t(1)),time2str(t(end)),'10',num2str(round(fnyq/2)),'1.0'};
...
phaseflag=3;
[amp,phs,tsd,fsd]=specdecomp(data,t,twin,tinc,fmin,fmax,delf,tmin,tmax,phaseflag); %#ok<ASGLU>
ind=near(t,tmin,tmax);
switch mode
    case 'inline'
        dname2=[strrep(dname,'plotimage3D ... ','') ' ' mode ' ' num2str(thisone)];
        pos=get(gcf,'position');
        datar=specd_viewer(data(ind,:),amp,t(ind),x,tsd,fsd,dname2);
        hppt=addpptbutton([.95,.95,.025,.025]);
        set(hppt,'userdata',dname2);
        axes(datar{1})
        xlabel('xline number');
...
tinc=2*dt;
fmt0=[.5*fnyq,mean(t)];
datar=seisplotfdom(data,t,x,twin,tinc,fmt0,dname2);
hppt=addpptbutton([.95,.95,.025,.025]);
set(hppt,'userdata',dname2);
colormap(cmap);
if(strcmp(gridon,'on'))
