function seisplota(seis,t,x,ampinfo,clplvl)
% SEISPLOTA ... creat a sesimic image plot in the current axes
%
% seisplota(seis,t,x,ampinfo,clplvl)
%
...
%       a four element vector [max min mean stddev]. This is useful when several different displays
%       need to have the same realtive amplitude. An entry of nan will invoke the default.
% ************ default is to compute these from the input data ************
% clplvl ... number of standard deviations from the mean at which to clip
% ************ default = 3 *******************
%
% NOTE: to have the default clipping set to "no clipping" use
% seisplota(seis,t,x,nan,inf)
%
...
if(strcmp(action,'init'))
    ...
    if(isnan(ampinfo))
        ampinfo=[max(seis(:)) min(seis(:)) mean(seis(:)) std(seis(:))];ampinfo=[max(seis(:)) min(seis(:)) mean(seis(:)) std(seis(:))];
    end
