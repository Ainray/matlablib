...
%
% hh=tvdbspec(t,seis,tnots,twins,tpad,dname,haxe,dbflag,indx)
%
...
% current axes or an axes which you desinate via the haxe parameter. If you want this in a
...
%vis=get(hfig,'visible');
%axes(haxe)
%set(hfig,'visible',vis);
if(dbflag==1)
...
if(time==1)
    ...
    legend('Total spectrum',[time2str(tnots(1)) ' to ' time2str(tnots(1)+twins(1)) ' sec'],...
        ...
        [time2str(tnots(3)) ' to ' time2str(tnots(3)+twins(3)) ' sec'],'location',legendloc);
    if(isempty(dname))
        enTitle('Time-variant spectra')
        set(gcf,'name','Time-variant spectra')
    else
        ht=enTitle({dname ' time-variant spectra'});
    ...
    if(isempty(dname))
        enTitle('Depth-variant spectra')
        set(gcf,'name','Depth-variant spectra')
    else
        ht=enTitle({dname ' depth-variant spectra'});
        set(ht,'interpreter','none')
...
function iwout=checkwindowsize(iw,t,imin)
if(length(iw)>imin)
    iwout=iw;
    return;
end
