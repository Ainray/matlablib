% function [w]=dft_hannpoisson(N,alpha,mode)
% author: Ainray
% date: v1,20160308
% version: 1.0
% bug-report: wwzhang0421@163.com
% introduction: calculating the hannpoisson window
%               w=0.5*(1+cos(2*pi/(N-1)*n))*exp(-a|n|/(N-1)/2,n=-(N-1)/2:(N-1)/2
%               Examples:
%                 w=dft_hannpoisson(10), return 11 samples of sysmmtric window
%                 w=dft_hannpoisson(10,'one-side'), return 10 samples window,
...
if strcmp(mode,'symmetric')
    ...
    n=[-(N-1)/2:(N-1)/2]';
    w=0.5*(1+cos(2*pi/(N-1)*n)).*exp(-alpha*abs(n)/floor(N/2));
else
